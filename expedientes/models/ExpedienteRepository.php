...
/**
    ...
    /**
     * Realiza una búsqueda en múltiples campos del expediente.
     * @param string $termino El término de búsqueda ingresado por el usuario.
     * @return array Retorna los expedientes que coinciden o un array vacío.
     */
    public function busquedaRapida(string $termino): array {
        // Preparar el término para la búsqueda LIKE
        $termino_like = '%' . $termino . '%';

        // Consulta que busca el término en número, letra, extracto, e iniciador
        // NOTA: Los nombres de columnas deben coincidir con tu esquema de DB.
        $sql = "SELECT * FROM expedientes 
                WHERE numero LIKE :q1 
                OR letra LIKE :q2 
                OR folio LIKE :q3 
                OR extracto LIKE :q4 
                OR iniciador LIKE :q5";
        
        try {
            $stmt = $this->db->prepare($sql);
            
            // Asignar el mismo término_like a todos los marcadores
            $stmt->bindParam(':q1', $termino_like);
            $stmt->bindParam(':q2', $termino_like);
            $stmt->bindParam(':q3', $termino_like);
            $stmt->bindParam(':q4', $termino_like);
            $stmt->bindParam(':q5', $termino_like);
            
            $stmt->execute();
            
            return $stmt->fetchAll();
        } catch (PDOException $e) {
            error_log("Error en busquedaRapida: " . $e->getMessage());
            return [];
        }
    }
